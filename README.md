...
let styleData = JSON.parse(localStorage.getItem("cal_style_vollure_rose")) || { style: "system", nightMode: false };
...
function saveStyle() {
  localStorage.setItem(STYLE_STORAGE_KEY, JSON.stringify({ 
    style: currentStyle,
...
function toggleScreenOrientation() {
  ...
  // 方案2：保留API调用但优化错误处理（注释掉，如需启用可取消注释）
  /*
  ...
  try {
    const target = screen.orientation.type.includes("portrait") ? "landscape" : "portrait";
    screen.orientation.lock(target).then(() => {
      toast(`已切换为${target === "landscape" ? "横屏" : "竖屏"}`);
    }).catch(err => {
      // 捕获所有错误，统一提示手动操作
      toast(`请手动将设备旋转为${targetOrientation}模式`);
    });
